...
#批量输入成绩，进行录入
@app.route('/piliangDaoRu', methods=['POST'])
def piliangDaoRu():
    xuehao = request.form.get('xuehao')
    kehao = request.form.get('kehao')
    kexuhao = request.form.get('kexuhao')
    ...
    #将数据存储到数据库中
    result = UpdateDataOne("update t_student_Course set achievement = "+str(ping)+", achievement_exam = "+str(kao)+" where student_num = '"+str(xuehao)+"' and course_num = '"+str(kehao)+"' and course_id = '"+str(kexuhao)+"'")
